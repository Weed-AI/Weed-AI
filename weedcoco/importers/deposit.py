...
def validate_duplicate_images(image_hash):
    if len(set(image_hash.values())) != len(image_hash):
...
def validate_existing_images(repository_dir, image_hash):
    addition_hash = set(image_hash.values())
...
def get_all_existing_hash(repository_dir):
    return reduce(
        lambda acc, it: acc.union(set(it[2])),
        filter(lambda dir: dir[0].endswith("/images"), os.walk(repository_dir)),
        set(),
    )
...
def setup_dataset_dir(repository_dir):
    if not os.path.isdir(repository_dir):
        ...
        if "dataset_0" in os.listdir(repository_dir):
...
def create_image_hash(image_dir):
    return {
        image_name: f'{get_image_average_hash(image_dir / image_name)}.{image_name.split(".")[-1]}'
...
def generate_weedcoco(weedcoco_path, dataset_dir, image_hash):
...
def main():
    ...
    generate_weedcoco(args.weedcoco_path, dataset_dir, image_hash)
